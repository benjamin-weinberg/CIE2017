...
{
  cout << "Character " << oneCharacter.getM_name() << " purchased: " << endl;
  double monthlyExpenditures = 0.0;
  for( auto purchase_elem : oneCharacter.getM_purchase_list() )
    {
    const string         store_name = purchase_elem->getM_store();
    const int            barcode = purchase_elem->getM_barcode();
    const Store *        curr_store = all_stores.at(store_name);
    const HealthStatus & curr_store_quality_multiplier = curr_store->getM_qualityMultiplier();

    const StoreInventoryItemMapType & curr_store_inventory = curr_store->getM_Inventory();
    const StoreInventoryItem *        curr_store_item = curr_store_inventory.at(barcode);
    const string &                    item_name = curr_store_item->getM_name();

    const HealthStatus itemHealthStatus = all_items.at(item_name)->getM_HealthStatus();
    const double       item_cost = all_items.at(item_name)->getM_price();

    oneCharacter.setM_HealthStatus(HealthStatus::HealthStatus( (itemHealthStatus.getM_food() ) *
                                                               (curr_store_quality_multiplier.getM_food() ),
                                                               itemHealthStatus.getM_clothes() *(
                                                                 curr_store_quality_multiplier.getM_clothes() ),
                                                               itemHealthStatus.getM_entertainment() *(
                                                                 curr_store_quality_multiplier.getM_entertainment() ) ) );

    cout << "     " << item_name << " at " << store_name << " for " << item_cost << endl;
    }

  cout << " Monthy total $" << std::fixed << std::setprecision(2) << std::setfill('0') << monthlyExpenditures
       << " resulting in health metrics of "
       << oneCharacter.getM_HealthStatus().toString() << endl;
  cout << "------------------------------------------------" << endl;
}
